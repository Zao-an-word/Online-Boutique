...
FAULT_PLAN = [
    ...
    {
        "id": "cpu-cart",
        "name": "CPU压力-cartservice(2核)",
        "desc": "cartservice 注入 2 核 CPU 100% 压力",
        "type": "stresschaos",
    ...
    {
        "id": "cpu-curr",
        "name": "CPU压力-currencyservice(2核)",
        "desc": "currencyservice 注入 2 核 CPU 压力（QPS 最高服务）",
        "type": "stresschaos",
    ...
    {
        "id": "cpu-front",
        "name": "CPU压力-frontend(2核)",
        "desc": "frontend 注入 2 核 CPU 压力（用户界面变慢）",
        "type": "stresschaos",
    ...
    {
        "id": "mem-reco",
        "name": "内存压力-recommendationservice(512MB)",
        "desc": "recommendationservice 注入 512MB 内存压力",
        "type": "stresschaos",
        "selector": {"app": "recommendationservice"},
        "stressors": {"memory": {"workers": 2, "size": "512MB"}},
        "duration": "90s",
    ...
    {
        "id": "mem-pay",
        "name": "内存压力-paymentservice(512MB)",
        "desc": "paymentservice 注入 512MB 内存压力",
        "type": "stresschaos",
    ...
    # ═══ Pod Kill — 让服务短暂消失 ═══
    {
        ...
        "desc": "杀死 checkoutservice（结账流程中断）",
        "type": "podchaos",
        "action": "pod-kill",
    ...
    },
    {
        ...
        "desc": "杀死 productcatalogservice（无法浏览商品）",
        "type": "podchaos",
        "action": "pod-kill",
    ...
    # ═══ Pod Failure — 让服务不可用（不重建）═══
    {
        "id": "fail-ship",
        "name": "PodFailure-shippingservice(60s)",
        "desc": "shippingservice 不可用 60 秒",
        "type": "podchaos",
        "action": "pod-failure",
    ...
    {
        "id": "fail-email",
        "name": "PodFailure-emailservice(60s)",
        "desc": "emailservice 不可用 60 秒",
        "type": "podchaos",
        "action": "pod-failure",
...
def kubectl_apply(yaml_str, name):
    ...
    result = subprocess.run(
        [KUBECTL, "apply", "-f", "-"],
        input=yaml_str, universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=30
    )
...
def make_yaml(fault_def):
    ...
    safe_name = fid.replace("_", "-")  # K8s 资源名不允许下划线
